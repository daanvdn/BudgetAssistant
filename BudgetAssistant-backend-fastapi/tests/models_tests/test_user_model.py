...
class TestUser:
    ...
    @pytest.mark.asyncio
    async def test_create_user_with_duplicate_email(self, async_session):
    ...
    @pytest.mark.asyncio
    async def test_retrieve_user_by_email(self, async_session):
    ...
    async def test_retrieve_non_existent_user(self, async_session):
        ...
        result = await async_session.execute(select(User).where(User.email == "nonexistent@example.com"))
    ...
    async def test_related_name_bank_accounts_to_users(self, async_session):
        ...
        assert bank_account.users[0].email == "test@example.com"
