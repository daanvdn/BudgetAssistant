...
@pytest_asyncio.fixture
async def bank_account_with_user(
...
class TestTransactionService:
    ...
    async def test_page_transactions_to_manually_review_returns_transactions(
        ...
        # Create counterparty
        counterparty = Counterparty(
            name="Test Counterparty", account_number="987654321"
        )
        async_session.add(counterparty)
        await async_session.flush()
    ...
    async def test_page_transactions_pagination(
        ...
        for page_num in range(10):
            ...
            expected_transaction_ids = [
                t.transaction_id for t in transactions[start_idx:end_idx]
            ]
    ...
    async def test_get_transaction_returns_transaction(
        ...
        # Create transaction
        transaction = await create_transaction(
            async_session, bank_account, counterparty
        )
        await async_session.commit()
    ...
    async def test_page_transactions_filters_by_counterparty_name(
        ...
        # Create transactions
        txn1 = await create_transaction(
            async_session, bank_account, counterparty1, transaction_number="TXN_ABC"
        )
        txn2 = await create_transaction(
            async_session, bank_account, counterparty2, transaction_number="TXN_XYZ"
        )
        await async_session.commit()
    ...
    async def test_get_distinct_counterparty_names(
        ...
        await create_transaction(
            async_session, bank_account, counterparty1, transaction_number="TXN_1"
        )
        await create_transaction(
            async_session, bank_account, counterparty1, transaction_number="TXN_2"
        )
        await create_transaction(
            async_session, bank_account, counterparty2, transaction_number="TXN_3"
        )
        await async_session.commit()
    ...
    async def test_page_transactions_filters_by_transaction_or_communication(
        ...
        txn1 = Transaction(
            transaction_id=Transaction.create_transaction_id(
                txn_id_1, bank_account.account_number
            ),
            bank_account_id=bank_account.account_number,
            booking_date=date.today(),
            statement_number="001",
        ...
        txn2 = Transaction(
            transaction_id=Transaction.create_transaction_id(
                txn_id_2, bank_account.account_number
            ),
            bank_account_id=bank_account.account_number,
            booking_date=date.today(),
            statement_number="001",
