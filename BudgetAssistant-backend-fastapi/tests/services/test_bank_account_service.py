...
class TestBankAccountService:
    ...
    async def test_get_or_create_bank_account_creates_new_account(
        ...
        bank_account = await service.get_or_create_bank_account(
            account_number, user, async_session
        )
    ...
    async def test_get_bank_account_returns_account(
        ...
        # Retrieve account
        retrieved_account = await service.get_bank_account(
            account_number, async_session
        )
    ...
    @pytest.mark.asyncio
    async def test_save_alias_updates_alias(
        self, async_session: AsyncSession, service: BankAccountService
    ):
    ...
    async def test_save_alias_raises_error_if_account_not_exist(
        ...
        assert "Bank account with account number nonexistent does not exist" in str(
            exc_info.value
        )
    ...
    async def test_get_account_numbers_for_user_returns_account_numbers(
        ...
        # Get account numbers
        account_numbers = await service.get_account_numbers_for_user(
            user, async_session
        )
